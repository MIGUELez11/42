...
/*   Updated: 2019/12/27 13:14:54 by miguelez1        ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		fix_sizes(t_fd_lst *this, int i, int npos)
{
...
char		*split_buffer(t_fd_lst *this, int npos)
{
	char	*aux_buf;
	char	*line;
	int		i;

	line = malloc(sizeof(char) * npos);
	if (!line)
		return (NULL);
	aux_buf = this->buffer;
...
/*
**	get_line(File descriptor to read, node we are reading from)
**
**	get_line finds a valid line, it checks if there is a valid line stored
