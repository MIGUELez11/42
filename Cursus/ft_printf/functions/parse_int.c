...
/*   Updated: 2020/02/03 19:52:46 by mflorido         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		parse_int(t_printf_list *p_lst)
{
	...
	if (p_lst->flags.width && !p_lst->flags.minus &&  p_lst->flags.width > len && p_lst->flags.width > p_lst->flags.precision)
	{
	...
	if((!p_lst->flags.minus && p_lst->flags.width > len && p_lst->flags.width > p_lst->flags.precision && p_lst->flags.zero) || (p_lst->flags.precision > len))
	{
		put_repeated_char('0', (p_lst->flags.width > p_lst->flags.precision ? p_lst->flags.width : p_lst->flags.precision) - len);
		p_lst->printed_chars += (p_lst->flags.width > p_lst->flags.precision ? p_lst->flags.width : p_lst->flags.precision) - len; 
	}
	putnbr_abs(n, 1);
