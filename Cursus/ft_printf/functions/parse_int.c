...
/*   Updated: 2020/01/30 18:42:33 by mflorido         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		parse_int(t_printf_list *p_lst)
{
	...
	len = ft_intlen(n);
//
	if (p_lst->flags.precision - len > 0)
		len = p_lst->flags.precision;
//
//
	if (p_lst->flags.width - len > 0 && !p_lst->flags.zero && !p_lst->flags.minus)
	{
...
//
	if (p_lst->flags.width - len > 0 && p_lst->flags.zero && !p_lst->flags.minus)
	{
		put_repeated_char('0', p_lst->flags.width - len);
		p_lst->printed_chars += p_lst->flags.width - len;
	}
//
//
	...
	{
		put_repeated_char('0', p_lst->flags.precision - ( n < 0 ? len + 1 : len));
		p_lst->printed_chars += p_lst->flags.precision - ( n < 0 ? len + 1 : len);
	}
//
	putnbr_abs(n, 1);
